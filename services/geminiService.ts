...
export const getThumbnailPrompt = (
  ...
  // 画像に入れる唯一の文字。出品文の生成時に Gemini が作る。
  // 古い出品文には無いので、その場合だけ画像生成側にルールつきで作らせる（作り直せば新しい形になる）
  const headlineMatch = content.match(/サムネイル見出し[：:]\s*(.*)/);
...
■ 要件：画像内にテキストは一切含めないこと。純粋なビジュアル表現のみ。プロフェッショナルで魅力的なデザイン。`;
  ...
  // ── 共通パート（全バージョン共通） ──
  // 載せる文字を「見出し1本」に絞る。文字が減れば、袋文字も座布団も色分けも要らなくなる。
  // 一覧ではタイトルが画像の外に文字で出るので、サービス名は画像に入れない。
  const headlineBlock = headline
    ? `【見出し】${headline}`
    : `【見出し】キャッチコピー「${catchphrase}」から、買い手が得るものを12〜16文字で言い切る見出しを1本作る。サービス名を入れない。数字は本文にあるものだけ。「サポート」「お手伝い」「丁寧に」のような中身のない語だけで作らない`;
  // 補助の一言は「入れない」を既定にする。文字が増えるほど、袋文字や座布団が要るようになるため
  const subLine = catchphrase
    ? `【補助】基本は入れない。見出しだけでは何のサービスか分からないときに限り、「${catchphrase}」から20文字以内に切り出した一言を、見出しより小さく1行だけ`
    : '';
  const commonBase = `あなたはプロのデザイナーです。スキルマーケット出品ページ用のサムネイル画像を1枚作ってください。
参考情報：サービス「${title}」、強み「${idea.strength}」。これは絵の題材を決めるための情報で、この文字を画像に入れてはいけない。

■ 画像に入れる文字（この文言だけを正確に。勝手に文章を足さない）
${headlineBlock}
${subLine}
・サービス名は画像に入れない（一覧ではタイトルが画像の外に文字で出るため）
・見出しは、画像の高さの10%以上の文字の高さで描く。幅300pxに縮めても読める大きさ
・日本語は誤字なく正確に

■ 主役
・このサービス固有のモチーフを1つ、参考情報から決めて大きく描く（道具・場面・成果物・相手のどれか）。人物にするか物にするかは内容で決める。提供者の代わりに動物やマスコットを立てない（サービスの題材が動物なら描いてよい）
・出品者のアイコン画像が添付されていれば、脇に自然に入れる。主役にはしない

■ 色
・背景1色・文字1色・アクセント1色の3色だけ。それ以外の色を足さない
・配色はサービスの雰囲気から決める
・アクセント色を使うのは1箇所だけ

■ 文字の飾り（守る）
・二重フチ・ドロップシャドウは使わない。背景とのコントラストで読ませる
・文字の下に色の帯（座布団）を敷くなら1枚だけ。見出し全体の下に1枚。単語ごとに敷かない
・強調のために色や大きさを変える語は、1語まで

■ 置かないもの
・下段の箇条書き、バッジの列、ベネフィットの列挙
・効果線、「！」、稲妻、キラキラ、放射線、ハーフトーン
・キャッチコピーやサービス名の再掲

■ 余白と構図
・画面の4割以上は何も置かない
・視線は「主役 → 見出し」の2手で終わる構図にする

■ 形式
・アスペクト比 3:2（横長）。660×440px 相当
`;
...
■ 共通：
・配色は背景・文字・アクセントの3色。見出しは大きく可読性優先。情報を詰め込みすぎない。`,
...
■ タイポグラフィ（文字デザイン）：
・親しみやすく、可読性の高い書体。
・背景のイラストに埋もれないよう、文字の下に紙の色の面を1枚だけ敷く。縁取りやドロップシャドウは使わない。

■ 装飾：
・デザインのテーマに沿った、小さく控えめな装飾イラストを複数配置し、画面に温かみや動きを加える。
・配色は水彩絵の具のにじみを活かした柔らかいトーンで、3色までに統一する。
...
・自然体で気負わない安心感。`,
    ...
    pop: `
■ トンマナ（世界観・雰囲気）：
・ポップ＆フレンドリー: 明るい色と丸みのある造形で、「楽しそう」と感じさせる、心理的ハードルの低い雰囲気。
・要素は詰め込まない。丸い形と大きな見出しで、軽さと期待感を出す。

■ デザインスタイル：
・配色は、白を少し混ぜた明るい色を3色まで。多色にしない。
・文字は背景との明度差で読ませる。縁取りを使うなら細い1重まで。ドロップシャドウは使わない。
・ハーフトーン（ドット）: 背景のグラデーションにドットパターンを重ねることで、漫画やアメコミのような「ポップアート」の質感を与える。
・動きは、モチーフの傾きや丸い形の重なりで出す。矢印やキラキラは使わない。
...
・参考画像のトーン・世界観を忠実に継承しつつ、新しいサービスの内容が自然に伝わるデザイン。`,
    ...
    youtube: `
■ デザインスタイル：YouTube風
・人気YouTuberのサムネイル風。顔や象徴物を大きく、見出しを太く、コントラストを強く。

■ 構図
・左に見出し（最大2行）、右に顔か象徴物。象徴物はサービス固有のものを参考情報から1つ。
・背景は2つの色面で分ける。それ以上は分けない。

■ 文字
・極太ゴシック。フチ取りは見出しにだけ、細い1重まで。二重にしない。
・強調する語は1語まで。塗り箱を使うなら1つ。

■ 色
・3色まで。高彩度の色は1色だけにして、残り2色は背景と文字に使う。

■ 置かないもの
・矢印、稲妻、星、キラキラ、放射線、手書き風の丸囲み。
・下段の箇条書きやバッジ。

■ 雰囲気
・思わずクリックしたくなる勢い。ただし、にぎやかさではなく、大きな顔と一言の強さで目を引く。`,
...
・初心者でも安心して頼めそうな優しい印象。`,
    ...
    benefit_banner: `
■ デザインスタイル：訴求バナー（ベネフィット型）
・販促バナー風。ひと目で「誰の何が、どう良くなるか」が伝わる、勢いと信頼感のあるフラットベクターイラスト調。
・実写・3D・複雑なグラデーションは使わない。面と極太の文字で構成する。

■ 主役の決め方（参考情報から。サービスごとに変える）
・提供者の人物か、サービス固有のモチーフか、どちらか1つを内容で選ぶ。人物なら半身で、前向きな一動作（指をさす・道具を持つ・画面を示す）。提供者の代わりに動物やマスコットを立てない（サービスの題材が動物なら描いてよい）。
・配色は3色。サービスの雰囲気で決める。テンプレの配色に寄せない。

■ 構図（2ゾーン）
・左：見出し。最大2行。画面の高さの10%以上の文字で。
・右：主役。画面の3分の1ほどの大きさで、余白を残す。
・背景に斜めの色面を入れるなら1本まで。それ以上の帯・ドット・放射線は入れない。

■ 文字
・極太ゴシック。フチ取りは無し。背景との明度差で読ませる。
・見出しの下に色の帯を敷くなら1枚だけ。強調する語は1語まで。

■ 雰囲気
・「この人なら任せられる」という信頼感と、思わず目が止まる言い切りの強さ。にぎやかさで引かない。`,
  };
...
・タイトルは30文字以内
・キャッチコピーは100文字以内
・「サムネイル見出し」はサムネイル画像に入れる唯一の文字。買い手が得るものを12〜16文字で言い切る（例「月8時間の手作業が、ゼロに」「経理の締めが、週明けに終わる」「毎月の集計、まだ手作業ですか」）。
　サービス名を入れない。数字は【自己紹介から抽出した事実】や本文にあるものだけ。「サポート」「お手伝い」「丁寧に」のような中身のない語だけで作らない。絵文字を使わない
・価格はカテゴリの一般相場を自動反映
...
・見出し行（カテゴリ：／サブカテゴリ：／タイトル：／キャッチコピー：／サムネイル見出し：／サービス詳細（…）／💰価格の目安／■ 標準価格／■ モニター価格／🔚さいごに／⚠️キャンセル時の注意事項／🎯出品者スキル／📝依頼テンプレート）は、絵文字・記号・文言ともにそのまま維持し、削除・改変しないこと。
・「💰価格の目安」ブロックがある場合、「■ 標準価格」→「■ モニター価格」の順・同じプラン構造は崩さないこと。編集指示が価格変更を求める場合のみ金額を変更してよい（モニター価格は標準価格のおよそ70%を目安に追随させる）。
・タイトルは30文字以内、キャッチコピーは100文字以内、サムネイル見出しは12〜16文字（サービス名なし・根拠のある言葉だけ）を維持すること。
・マークダウンの書式（# や ** など）は一切使わないこと。
