...
・親しみやすさと勢いを両立した"エンタメ寄り"の印象。`,
    ...
    pivot: `
■ デザインスタイル：
・ビジネス情報メディア「PIVOT」のような、洗練されたインタビュー番組トンマナ。
・知的で上質、ビジネスパーソン向けの落ち着いた大人のデザイン。

■ レイアウト構成：
・人物写真（ゲスト／出品者）を画面の右半分または中央に大きく配置し、主役にする。
・タイトルとキャッチコピーは左側または下部に、読みやすく整列させる。
・左上にブランドタグ風の小さなラベル（例：番組名・カテゴリ）を配置。

■ タイポグラフィ（文字デザイン）：
・大きく効かせた欧文セリフ／ゴシック、または和文ゴシックの組み合わせ。
・文字装飾（縁取り／影）は最小限、レイヤー感ではなくカーニングと余白で勝負。
・強調は色ではなく文字サイズ・ウェイト差で表現。

■ カラー：
・深いグリーン系（フォレストグリーン／ダークティール）または濃紺をベースに、ホワイトをアクセントに。
・モノトーン寄りで彩度は抑えめ、2〜3色に絞る。

■ 装飾：
・装飾は極力排除。余白を贅沢に使う。
・グラデーション・縁取り・ドットパターン・ポップ要素は使用しない。

■ デザインの雰囲気：
・上品・知的・洗練された"ビジネスインタビュー番組"の印象。
・静かな説得力と、視聴者への信頼感を醸成する落ち着いたトンマナ。
・余白の美しさと端正な組版で、サービスの品質を象徴する。`,
  };
...
export const getSlideDocPrompt = (
  ...
  const bodyExcerpt = serviceBody?.trim() || '(リソースに登録されたサービス詳細文章を参照してください)';
...
`,
    pivot: `■ デザインスタイル：PIVOT風
・ビジネス情報メディア「PIVOT」のような、洗練されたインタビュー番組トンマナ
・深いグリーン系（フォレスト／ダークティール）または濃紺ベースにホワイトをアクセント
・人物写真（ゲスト・出品者）を画面の主役に大きく配置
・左上にブランドタグ風の小さなラベル（番組名・カテゴリ）
・装飾は極力排除、余白を贅沢に使いタイポグラフィと人物写真で勝負
・知的・上品・静かな説得力のあるビジネス向けデザイン
`,
  };
